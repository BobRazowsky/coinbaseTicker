...
var browser = (<any>window).browser;
...
function initializeConfig(configuration){
    ...
    if (typeof localStorage.beenHereBefore === "undefined") {
        ...
        localStorage.setItem("delay", (configuration.updateDelay * 1000).toString());
...
function getJSON(url: string, callback){
...
function setBadgeColor(color: string){
    browser.browserAction.setBadgeBackgroundColor({color: color});
}

function createNotification(sentence: string, value: number){
